...
To study the differences between adult and neonatal fibroblasts and their response to \tgf{}, an experiment was conducted to measure the activities of 72 genes over 96h by high throughput qPCR. This experiment was conducted in neonatal and adult cell lines in both control and \tgf{} treatments. Moreover, given that an increase in the prevalence of senescence is a feature of ageing (ref), an irradiation-induced senescent cell line model of replicative senescence was similarly investigated. Ageing is known to be a variable process, with considerable heterogeneity at the population level (ref). In attempt to account for some of this variability, three replicates each for neonatal, adult and senescent cell lines were investigated. In addition to treatment with a negative control and \tgf{}, a baseline condition was also introduced. These cells were not treated, but harvested at the start and end of the experiment. Each of the resulting 216 individual conditions were replicated 6 times (\cref{fig:exp_design}). 

\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{img/qc/cell_line}
	\caption{Scatter matrix of first three principle components coloured by cell line. Kernel density estimates along the diagonal represent the distribution of the relevant principle component.}
	\label{fig:qc:cell_line}
\end{figure}

%\subsubsection{Principle component analysis}
The amount of IL1A, IL1B and FOSB were either present at quantities below the range of detection or unreliable primers. These were excluded from further analysis. The data for the remaining samples were used in a principal component analysis (PCA). The scree plot (\cref{fig:qc:scree}) indicates that the first three principal components account for >80\% of variance and so a 3x3 scatter plot matrix was produced to visualise the first three principal components of this dataset.


 Alternatively, an interactive data visualisation web application is available for viewing the PCA in 3 dimensions. In this PCA, each point represents one of the 1296 individual samples. The closer together two points lie, the more similar the these samples are to each other. 

In \cref{fig:qc:cell_line}, the PCA has been coloured by cell group and clearly shows that cells from the same group are more similar to each other than cells of another group. Similarly, samples that were from the same cell ID (\cref{supp:fig:qc:cell_id}, time (\cref{supp:fig:qc:time_point}) and treatment (\cref{supp:fig:qc:treatment}) clustered together while replicates (\cref{supp:fig:qc:replicate}) did not. Collectively, these figures indicate that there are differences between how neonatal, senescent and adult cells behave. 

%\subsubsection{Differential expression}
To investigate where the differences lie between the three cell types, a differential expression analysis was conducted using LIMMA. LIMMA is an R package designed for computing statistics on microarray and RNA-seq datasets (ref the limmma papers), however, provided any data preprocessing is conducted manually, LIMMA can also be used computing statistics on high throughput qPCR datasets.

Our primary interest was to illuminate any differences between the behaviour of aged/senescent with neonatal fibroblasts, which were used to benchmark normal healthy fibroblasts. These comparisons are referred to as `between groups' comparisons to differentiate them from the `within group' comparisons whereby a cell line is compared with other cell lines in its category. 

Because our data is dynamic, LIMMA was configured to evaluate whether a full time series is statistically different from another full time series by conducting a moderated F-test, as implemented in LIMMA \cite{Smyth2004}. To make use of the fact that we have data from 3 cell lines per category, all combinations of relevant cell line were compared with all other relevant cell lines per treatment (\cref{fig:stats:diagram} and the percentage of times each gene was considered differentially expressed (FDR corrected p-value < 0.001) was computed. The resulting percentages are presented in \cref{fig:heatmap}. If a gene was differentially expressed in >60\% of the LIMMA analyses in a comparison, the gene was considered differentially expressed overall, a summary of which is presented in \cref{table:stats_summary}.
...
\begin{figure}
	...
	\begin{subfigure}{0.45\linewidth}
		\includegraphics[height=0.7\textheight, width=\linewidth]{LIMMA09_2018/SavedObjects/analysis_at_pval_less_than_0_001/between_heatmap0_001_2}
		\caption{Between groups comparisons}
		\label{fig:between_heatmap}
	\end{subfigure}
	\begin{subfigure}{0.45\linewidth}
%		\centering
		\includegraphics[height=0.7\textheight,width=\linewidth]{LIMMA09_2018/SavedObjects/analysis_at_pval_less_than_0_001/within_heatmap0_001_2}
		\caption{Within groups comparisons}
		\label{fig:within:heatmap}
	\end{subfigure}
	\caption{Heat maps depicting how often of all the differential expression analyses a gene was differentially expressed at a (FDR corrected) p-value smaller than 0.001.}
	\label{fig:heatmap}
\end{figure}
...
\begin{figure}[p]
	...
	\end{minipage}
	\caption{Bar graphs showing control (C) and \tgf{} (T) treated time series datasets for each cell line. Error bars represent standard error of the mean.}
	\label{fig:bar_graphs}
\end{figure}

%\subsubsection{A selection of gene measurements with detailed temperol dynamics}
\cref{fig:bar_graphs} shows a selection of the genes measured in this experiment. For a pdf document containing A4 sized graphs of all measured genes please see supplementary file 1. Alternatively, all data has also been built into an web application for interactive data visualisation.
...
$\upalpha$-SMA (ACTA2) and connective tissue growth factor (CTGF) were both among the most strongly activated of the genes measured. \tgf{} strongly induced ACTA2 in neonatal and adult cell lines but only weakly in adult cell lines (\cref{ACTA2}). CTGF also responded strongly to \tgf{} and adult cell lines produced less CTGF than neonatal cell lines (\cref{CTGF}). The amount of CTGF produced without \tgf{} however was very small. 

Senescent cell lines produced more MMP1 than neonatal cell lines while adult cells variably produced more than both neonatal and senescent cell lines \cref{MMP1}. The amount of MMP1 produced in all cell lines started high under the baseline 0h condition and decreased over time. \tgf{} exerted no effect on MMP1. 

Smad7 is present in very small quantities without the presence of \tgf{} while \tgf{} treatment dramatically induces Smad7 transcription (\cref{SMAD7}). The Smad7 response is transient, peaking at approximately 2h post stimulation and then the amount produced reduces again. While some variability exists, particularly in adult cell lines, the response to Smad7 is similar in each cell line.

Smad3 transcription remains unchanged without the presence of \tgf{}. However, in all cell lines tested, \tgf{} induced a reduction in Smad3 production between after 8h of \tgf{} stimulation. 
...
\subsubsection{Interactive web application for data visualisation}
The data accumulated in this experiment is available for viewing and download at \url{http://cwelsh2.pythonanywhere.com/}. The purpose of this website is to host the data and enable readers to interactively explore this dataset. Options are available for comparing different genes, cell lines, treatments and time points. The full dataset or any subset of the data are available for visualisation as a data table as well as for download from the website. Moreover, an interactive, 3 dimensional PCA for this dataset is available for exploration. The PCA data can be subset using the available options and coloured by the various experimental factors or combinations thereof. 

\section*{Discussion}
Biological tissues from older individuals operate at a reduced capacity when compared to young tissue. When skin ages it becomes thinner, lax, weaker and less elastic compared with young skin. Collectively these phenotypes contribute towards the more general phenotype of aged skin. Given that aged skin is inferior to young skin, therapeutic and cosmetic efforts have been focused on skin rejuvenation to promote healthy ageing (refs).

An issue with innovating a new means of skin rejuvenation therapies is that it is not yet clear  precisely where the differences lie between young and old tissue. While several papers are dedicated to the study of type 1 collagens in age (refs), many other ECM proteins have received little attention. Since optimal skin function manifests from an appropriate balance of ECM proteins it stands to reason that to identify therapeutic targets for rejuvenation, it would be prudent to first identify the differences between young and old tissue. It is to this issue that the current study is dedicated. 

To identify differences between neonatal and adult fibroblasts, we have designed a high throughout qPCR experiment to measure the activities of 72 genes over time in both a \tgf{} stimulated and unstimulated condition. While 72 genes is by no means a comprehensive accounting of the potential differences between these cell lines, these genes were selected based on their known involvement in ECM and \tgf{} biology. It is clear from the PCA that we have observed marked differences in the behaviour of neonatal, adult and senescent cells \cref{fig:qc:cell_line}. Additionally, the PCA serves to confirm that cells treated with \tgf{} behave differently to those treated with a negative control (\cref{fig:qc:treatment} an that the longer cells are treated, the more different they become, compared to baseline and control samples (\cref{fig:qc:time_point}. Moreover, the PCA does indicate replicate clustering which would be indicative of a confounding factor affecting the results \cref{fig:qc:replicate}.
